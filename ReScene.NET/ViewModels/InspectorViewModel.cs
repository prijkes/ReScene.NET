...
public partial class InspectorViewModel : ViewModelBase
{
    ...
    {
        if (SelectedTreeNode?.Tag is not SrrStoredFileBlock stored || _loadedFilePath == null || _srrData == null)
            return;
        ...
        try
        {
            await Task.Run(() =>
            {
                ...
                _srrData.SrrFile.ExtractStoredFile(_loadedFilePath, dir,
    ...
    private void SetHexBlock(long offset, long size)
    {
        ...
        HexBlockLength = (int)Math.Min(size, int.MaxValue);
    }
    ...
    private void AddProperty(string name, string value, ByteRange? range = null, bool indented = false)
    {
    ...
    private void ShowDetailedBlockProperties(RARDetailedBlock block)
    {
        foreach (var field in block.Fields)
        {
            ...
            AddProperty(field.Name, value, range);
